...
bundledir = "/tmp/sensorbundle"
if path.exists(bundledir):
    rmtree(bundledir)
if path.exists("/tmp/capture.pcap"):
    remove("/tmp/capture.pcap")
mkdir(bundledir)
...
def ona_settings_and_logs():
    ...
    run(
        [
            "/opt/silk/bin/rwcut --timestamp-format iso --fields sIp,dIp,sPort,dPort,protocol,Bytes,Packets,sTime,eTime /opt/obsrvbl-ona/logs/ipfix/.202* >> /tmp/sensorbundle/ona_settings/logs/ipfix/clear_silk 2>/dev/null"
        ],
        shell=True,
    )
...
def disk_stats():
    ...
    cmd_to_file(f"{bundledir}/disk_stats/lsof", "timeout 60 lsof")


def pcap_30_second():
    run(
        [
            "timeout 30 tcpdump -i any not host localhost and not host 127.0.0.1 and udp -c10000 -w /tmp/capture.pcap >/dev/null 2>&1"
        ],
        shell=True,
    )
...
def main():
    ...
    cmd = f"tar -Jcf {bundle_name} -C /tmp/sensorbundle . ../capture.pcap --remove-files 2>/dev/null"
