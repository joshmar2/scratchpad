...
def upload_file(case, token, f_name):
    if rqst_avail:
        try:
            ...
            print_log(f"[FAILURE] Failed to upload Failed to upload `{f_name}` to {case} with token {token}.", screen=True, color="red" )
        ...
        try:
            ...
            if output:
                print_log(f"[FAILURE] (cURL) Failed to upload `{f_name}` to {case} with token {token}.", screen=True, color="red")
...
def main():
    functions = [set_stage, ona_meta_data, get_ip, os_info, network, connectivity, ona_settings_and_logs, process_info, disk_stats, pcap_1_second]
    ...
    run(cmd, shell=True)
    if args.command == "upload": 
        unique_bundle_name = f"{bundle_name.replace('.tar.xz', '')}_first_responder.tar.xz" 
